...
it('shows only the authenticated user tokens in the table', function (): void {
    ...
    $this->actingAs($userA);
    $tokensA = collect(range(1, 2))->map(fn () => $userA->createToken('Token A '.Str::random(4))->accessToken)->map(fn ($token) => PersonalAccessToken::query()->find($token->id));

    $this->actingAs($userB);
    $tokensB = collect(range(1, 2))->map(fn () => $userB->createToken('Token B '.Str::random(4))->accessToken)->map(fn ($token) => PersonalAccessToken::query()->find($token->id));
